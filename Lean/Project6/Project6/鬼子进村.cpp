...
void qh(int f, int ne, int poi)/*计算要找的区间中完好的房子数*/
{
...
void search1(int f, int ne, int poi)//查找左端最近的一个零 
{
    ...
    {
        cc1 = 0;//如果找到最后，发现左边所有的点都是1，那么最左边记为0 
        return;
    }
...
void search2(int f, int ne, int poi)//对于寻找在询问点右边最近为零的点
{
    if (T[poi].l == f && T[poi].r == ne && T[poi].v == 0 && f == ne)
    {
        cc2 = f;
